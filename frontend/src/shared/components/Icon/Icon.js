...
export function renderIcon(name, options = {}) {
  ...
  icon.setAttribute('class', getClassName('ui-icon', options.className));
  ...
  definition.shapes.forEach(function (shape) {
    icon.appendChild(renderShape(shape));
  });
...
function renderShape(shape) {
  const shapeName = shape.tag || 'path';
  const element = document.createElementNS(SVG_NS, shapeName);

  Object.keys(shape).forEach(function (attribute) {
    if (attribute !== 'tag') {
