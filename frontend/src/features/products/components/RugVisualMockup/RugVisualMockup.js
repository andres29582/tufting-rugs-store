...
function getSvg(rug) {
  const id = rug.slug.replace(/[^a-z0-9-]/g, '');
...
export function renderRugVisualMockup(rug) {
  const shell = document.createElement('div');
  shell.className = 'rug-visual-shell';
  shell.style.setProperty('--rug-a', rug.colors[0]);
  shell.style.setProperty('--rug-b', rug.colors[1]);
  shell.style.setProperty('--rug-c', rug.colors[2]);
  shell.innerHTML = [
    '<div class="rug-glow" aria-hidden="true"></div>',
    '<div class="rug-shadow" aria-hidden="true"></div>',
    getSvg(rug)
  ].join('');
